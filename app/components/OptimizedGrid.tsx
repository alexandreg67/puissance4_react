...
const GridComponent: React.FC<OptimizedGridProps> = ({ 
  ...
  return (
    <div className={`flex flex-col items-center ${spacingConfig.sectionSpacing} w-full max-w-full`}>
      ...
      {/* Futuristic Column Indicators - Desktop only for cleaner mobile experience */}
      {deviceType === 'desktop' && (
        <div 
        ...
        >
          {Array.from({ length: 7 }, (_, index) => (
            ...
            >
              ...
              <div className="hud-panel text-neon-cyan border-neon-cyan rounded-full font-mono font-bold tracking-wider opacity-60 group-hover:opacity-100 transition-opacity px-3 py-1 text-xs">
                {index + 1}
              </div>
              
              {/* Drop zone indicator */}
              <div className="bg-gradient-to-r from-transparent via-neon-cyan/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity animate-neon-pulse h-1 w-8"></div>
            </div>
          ))}
        </div>
      )}
